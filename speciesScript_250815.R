...
downloadnew = FALSE # if true all dinosaur data are downloaded again from PBDB. if false data are loaded from file
...
##  
# Printing figures
load("Analysis_Genera.RData")
load("Analysis_Species.RData")
dospec = 1; #yaxis label 1- species richness 2 - genus richness
docis=TRUE # if plotting the confidence interval of the sampling data.
printfilename = 'Species_Sampling_pchlty_CI.jpg'
jpeg(printfilename,width=18,height=8,units='cm',res=1200)
# source('SamplingFigures_fordist.R')
source('SamplingFigure_diffpchlty.R')
dev.off()
# source('RichnessFigure_for_distribution.R')
printfilename = 'GeneraRichness_I.jpg'
jpeg(printfilename,width=18,height=14,units='cm',res=1200)
dospec=FALSE
source('RichnessFigure_withRT.R')
dev.off()

printfilename = 'AllSamplingratesforSI.jpg'
jpeg(printfilename,width=18,height=14,units='cm',res=1200)
