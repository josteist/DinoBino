...
plbins = 1:27;
ylimits = c(-.1,max(p_int_median,na.rm=TRUE))
xlimits = rev(range(midpoints[plbins]))
# Need to loop over polygons here since some are NA's.
par(mar=(c(0,4,4,2)))
ii=1
plot(midpoints[plbins],p_int_median[plbins,ii,1], type='o',col=color.list[1],ylim=ylimits,xlim = rev(range(midpoints[plbins])),xlab="Ma",ylab=expression(paste(lambda, " - sampling rate")),xaxt="n")
# for (ii in 1:4){
abline(h = 0,col="lightgray",lwd=0.5)
for (ii in 1:4){
...
for (jj in plbins){
  lines(rep(Bins[jj,1],2),c(-.2,ylimits[2]*1.2),col="lightgray",lwd=.5)
}
lines(rep(Bins[1,2],2),c(-.2,ylimits[2]*1.2),col="lightgray",lwd=.5)
for (jj in 1:6){
  lines(rep(max(Bins[Bins[,6]==jj,1]),2),c(-.2,ylimits[2]*1.2),col="grey10",lwd=0.5)
}
lines(rep(min(Bins[Bins[,6]==6,2]),2),c(-.2,ylimits[2]*1.2),col="grey10",lwd=0.5)
...
par(mar=(c(4,4,0,2)))
ylimits = c(-.2,1)
plot(midpoints[plbins],p_binos[plbins,1,1], type='o',col=color.list[1],ylim=ylimits,xlim = rev(range(midpoints[plbins])),yaxt="n",ylab = "Binomial sampling probability",xlab="Ma",xaxt="n")
...
for (jj in plbins){
  lines(rep(Bins[jj,1],2),c(0,1.2),col="lightgray",lwd=.5)
}
lines(rep(Bins[1,2],2),c(0,1.2),col="lightgray",lwd=.5)
for (jj in 1:6){
  lines(rep(max(Bins[Bins[,6]==jj,1]),2),c(0,1.2),col="grey10",lwd=0.5)
}
lines(rep(min(Bins[Bins[,6]==6,2]),2),c(0,1.2),col="grey10",lwd=0.5)
...
for (ii in 1:27){
  rect(Bins[ii,1],-.1,Bins[ii,2],0,lwd=0.3,col = color.list.eps[Bins[ii,6] %% 2])
}

for (ii in 1:6){
  rect(max(Bins[Bins[,6]==ii,1]),-.2,min(Bins[Bins[,6]==ii,2]),-.1,lwd=0.3,col = color.list.eps[ii %% 2])
}
for (ii in 1:27){
